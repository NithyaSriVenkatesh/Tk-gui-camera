...
def create_gui():
    ...
    def switch_to_camera_view():
        ...
        stop_event.clear()
        threading.Thread(target=handle_video_stream, args=(label_camera, stop_event)).start()
    ...
    btn_select_camera = tk.Button(frame_home, text="Select Camera", command=switch_to_camera_view)
    ...
    btn_quality_low = tk.Button(frame_camera, text="Low Quality", command=adjust_quality_low)
    btn_quality_low.pack(side="left", padx=10, pady=10)

    btn_quality_high = tk.Button(frame_camera, text="High Quality", command=adjust_quality_high)
    ...
    btn_gimbal_up = tk.Button(frame_gimbal, text="Up", command=lambda: control_gimbal("up"))
    btn_gimbal_up.grid(row=0, column=1)

    btn_gimbal_down = tk.Button(frame_gimbal, text="Down", command=lambda: control_gimbal("down"))
    btn_gimbal_down.grid(row=2, column=1)

    btn_gimbal_left = tk.Button(frame_gimbal, text="Left", command=lambda: control_gimbal("left"))
    btn_gimbal_left.grid(row=1, column=0)

    btn_gimbal_right = tk.Button(frame_gimbal, text="Right", command=lambda: control_gimbal("right"))
    btn_gimbal_right.grid(row=1, column=2)

    btn_gimbal_home = tk.Button(frame_gimbal, text="Home", command=lambda: control_gimbal("home"))
